...
async function getTaskDetails(taskId: string) {
  const response = await axios.get(
    `${Backed_URL}/api/v1/user/task?taskId=${taskId}`,
    {
      headers: {
...
export default function Page({
  ...
  return (
    <div>
      <Appbar />
      {/* //TODO: title of the task can be returned with the details only */}
...
function Task({ imageUrl, votes }: { imageUrl: string; votes: number }) {
  return (
    <div>
      <img key={imageUrl} className={"p-2 w-96 rounded-md"} src={imageUrl} />
